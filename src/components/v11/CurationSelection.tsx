...
export function CurationSelection() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setIsLoading(true);
            try {
                // We fetch all items to find the one we need (simpler for now than adding getItemById)
                // or we could add getItemById to the service layer.
                // Let's just find it in the list for now.
                const [itemsData, outfitsData] = await Promise.all([
                    api.getItems(),
                    api.getOutfitsByItem(itemId)
                ]);

                const foundItem = itemsData.find(i => i.id === itemId);
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-display font-medium text-white/40 tracking-widest uppercase">Deep Diving</p>
                </div>
            </div>
        );
    }
                        ...
                        >
                            <div className="text-left">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-lg font-bold text-white/20">Rs. {Number(item.price).toLocaleString()}</span>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <div className="absolute inset-x-0 bottom-0 p-8 flex flex-col items-start translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <img src={outfit.stylist?.avatar_url} className="w-6 h-6 rounded-full grayscale object-cover ring-1 ring-white/10" />
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="px-6 py-4">
                ...
                <div className="space-y-6">
                    {outfits.map((outfit) => (
                        ...
                        >
                            <img src={outfit.model_image_url} className="absolute inset-0 w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/20 to-transparent" />
                            <div className="absolute inset-x-0 bottom-0 p-8">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <img src={outfit.stylist?.avatar_url} className="w-5 h-5 rounded-full grayscale object-cover ring-1 ring-white/10" />
                                        <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">{outfit.stylist?.name}</span>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
